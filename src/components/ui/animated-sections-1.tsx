// components/ui/animated-sections-1.tsx
...
const AnimatedSections: React.FC<AnimatedSectionsProps> = ({
  ...
  return (
    <div 
      ref={containerRef}
      className={cn("h-screen w-screen overflow-hidden bg-black text-white uppercase font-sans", className)}
    >
      ...
      <div className="fixed bottom-4 right-6 z-30 flex items-center gap-4">
        <div className="flex gap-2">
          {sections.map((section, i) => (
            <button
              type="button"
              key={`thumb-${section.text}-${i}`}
              className="w-12 h-8 rounded overflow-hidden relative cursor-pointer transition-transform duration-300 p-0 border-0 bg-transparent"
              onClick={() => {
