...
- **FR-006**: Webhook handler validates signature/nonce/timestamp, ensures clock-skew tolerance, and rejects stale/invalid callbacks.
- **FR-007**: Webhook processing is idempotent (idempotency key per event/order) and persists status updates atomically.
- **FR-008**: Expose health/diagnostic endpoint covering G2A connectivity (token fetch probe) and idempotency store availability.
